...
For example, direct pass through diagonal with width $w:= \frac{|p|}{k} - |p|*k = |p|(\frac{1}{k} - k)$ in $H^{str-sub}_{p,t}$ (\todo{?}see fig) or in $M$ (\todo{?}see fig).
Another approach is the following.
Note, $M$ is a Monge matrix where indices are swapped with respect to matrix $H^{str-sub}$.
\todo{It allows us to descry this diagonal as approximately $|t|$ square windows of size $w \times w$ i.e a sliding 2d window of step 1 that goes diagonally.}
...
Each of these \todo{$W:=wxw$} matrices is Monge by definition, as a submatrix of a Monge matrix.
...
\todo{It is just pass through permutations that requires $O(v*|t|)$.}
...
Also, in our case random access query is $O(|t|)$ \todo{and?}.
...
\todo{Precisely, to the cell that represents substring $t_{0,|p|*k}$, starting at zero position and ending in $|p|*k$ position.}
