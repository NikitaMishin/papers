...
Semi-local lcs can be solved within the optimal running time compexity, $O(|a| \times |b|)$, by a number of algorithms (\todo{such as...})~\cite{.}.
...
First, note that the scoring scheme from~\ref{formula:sa} may be simplified by so-called normalization~\cite{.}:\begin{equation}\label{weightNormalization}
  \begin{array}{ll}
    w &= (w_{+}, w_{0} , w_{-}) = (w_{+} +2x , w_{0} + 2x , w_{-} + x), \forall x\footnote{\todo{...}}\\
...
\todo{In general, this data structure cannot be constructed in time less than $O(n^2)$ since storing matrix $M$ requires al teast $O(n^2)$ space.}
...
\todo{There have been several researches over several decades about rmq on Monge matrices \cite{}.}
...
\begin{theorem}~\cite{.}
Given an $n \times n$ staircase\footnote{\todo{Defintion}} Monge matrix $M$, a data structure of size $O(n)$ can be constructed in $O(n \log n$) time to answer submatrix maximum queries in $O(\log \log n$) time when random access to Monge matrix is $O(1)$.
\end{theorem}

\begin{theorem}~\cite{.}
Given an $n \times n$ partial Monge matrix\footnote{\todo{Definition}} $M$, a data structure of size $O(n)$ can be constructed in $O(n \times \log n$) time to answer submatrix maximum queries in $O(\log \log n$) time when random access to Monge matrix is $O(1)$.
\end{theorem}
...
\todo{More precisely, within each of the element of $W_{1}$ the largest one substring with legnth fall in $I$ that most similar to pattern $p$ according to $d_{di}$ is selected.}
\todo{The set $W_{2}$ is a result of this phase.}
...
\todo{The first phase requires at most $O(|t| \times |p|^2)$ due to fact that computing cost of edit distance is $|p|^2$ for strings of size $O(|p|)$ and we need to process $O(|t|-\frac{|p|}{k})=O(|t|)$ windows.}
The \todo{cardinality} of set $W_{1}$ at worst case is $O(|t|)$.
Thus, the first loop of the second phase will be iterated over $O(|t|)$ times.
The second loop refers to iteration over $I$ set with cardinality $O(\frac{|p|}{k}-|p|k) = O(|p|)$.
The third loop requires at most $O(|p|)$ \todo{?} since we again goes with sliding window.
The compare operation requires at most $O(|p|^2)$ \todo{?} since both strings are of size $O(|p|)$.
