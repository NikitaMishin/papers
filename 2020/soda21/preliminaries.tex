...
\red{
One example is when it's applied to biology where substrings with small size may be less important in a biological sense then substrings with less similarity score and much larger length.
Same true for upper bound on the length.
The other example is clone detection when you want to decrease the false-positive rate}.
...
Semi-local lcs can be solved within the optimal running time compexity, $O(|a| \times |b|)$, by a number of algorithms (\red{such as one that uses fast distance multiplication of unit-monge matrices~\cite{} or one that uses seaweed combing   }~\cite{.}).
...
\red{Finally, the blown-up technique\footnote{Each of the input length increase by factor $v$} is applied after reducing scoring scheme.
At this stage we have transitioned to \emph{semi-local lcs} problem which can be solved in $O(|a||b|v^2)$ time by algorithm\cite{.}.
The usage of second one algorithm allows to reduce of the one $v$}.
...
\red{In general, optimal space complexity of this data structure is $O(n^2)$  since storing matrix $M$ requires al teast $O(n^2)$ space.}
...
\red{There have long decades of research about \emph{rmq} on Monge matrices \cite{}.}
...
\begin{theorem}~\cite{.}
Given an $n \times n$ partial\footnote{Partial matrix is the matrix where some entries undefined, but defined entries in each coloumn and row are contigous} Monge matrix\footnote{\red{A partial Monge matrix is  partial matrix where the Monge property  is holds true for every four defined entries in Matrix}} $M$, a data structure of size $O(n)$ can be constructed in $O(n \times \log n$) time to answer submatrix maximum queries in $O(\log \log n$) time when random access to Monge matrix is $O(1)$.
\end{theorem}
...
\red{
More precisely, each element $w \in W_{1}$ is iterated over
to find the longest one substring in $w$ with lenght fall $I$ in interval that is most similar to pattern $p$.}
\red{
After these phase we have set $W_{2}$}
...
\red{
The running time complexity of the first phase is $O(|t| \times |p|^2)$ since there will be at proccessed at most
$O(|t|-\frac{|p|}{k})=O(|t|)$ windows with size $L_{w}$ 
and running time complexity of edit distance is $O(|p|^2)$
}.
Note that cardinality of set $W_{1}$ could be meaused as $O(|t|)$ at worst case.
\red{Thus, the the first loop of the second phase will be iterated over $O(|t|)$ times. (Line 8)
The second loop (Line 10) refers to iteration over $I$ set with cardinality $O(\frac{|p|}{k}-|p|k) = O(|p|)$.
The third loop (Line 11) requires at most $O(|p|)$ iterations  since we again goes with sliding window.
The running time complexotity of  \emph{Compare} operation(Line 12) is at most $O(|p|^2)$ since both strings are of size $O(|p|)$.
Thus, the total running time compleixty of the second phase is $O(|p|^4|t|)$ at worst case.
